...
protoc \
    ...
    -I googleapis \
    --openapi_out=. \
    --openapi_opt=fq_schema_naming=true,default_response=false,title="Orchestrator Service",version=v1,description="This is the OpenAPI schema for Orchestrator gRPC API",naming=json \
    ${QCONTROLLER_LOCATION}/src/protos/services/orchestrator/v1/service.proto

find ${OUTPUT_DIR}/protos/* -type d -exec touch {}/__init__.py \;

# Generate OpenAPI client for orchestrator service
openapi-generator-cli generate -g python -i openapi.yaml --additional-properties=generateSourceCodeOnly=false,packageName=orchestrator_service -o ${OUTPUT_DIR}/orchestrator
